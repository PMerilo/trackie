...
export default function TaskCard({ 
    ...
    function addStep() {
        ...
        stepApi("put", {content: '', completed: false, taskId: task.id})
        .then((res) => {
            res.json()
            .then( newStep => {
                setSteps(steps => steps.map((s, i) => i == steps.length-1 ? { ...s, id: newStep.id }: s))
            })
        })
    }
            ...
            <div className="collapse-content"> 
                <div className="mt-6 flex flex-col gap-y-4">
                    <div className="form-control">                
                        { steps.map((s, i) => <TaskStep checked={s.completed} value={s.content} onBlur={updateStep} onDelete={delStep} index={i} key={i}/>)}
                        <button role="button" className="btn btn-block justify-start" onClick={addStep}>
                            Add step +
                        </button>
                    </div>
                    <div className="flex justify-end gap-x-4">
                        <button className="btn bg-slate-500" onClick={() => console.log("test")}>Edit</button>
                        <button className="btn btn-error" onClick={() => console.log("test")}>Delete</button>
                    </div>
                </div>
            </div>
        </div>

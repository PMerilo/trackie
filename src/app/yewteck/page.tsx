...
export default function Home() {
  ...
  const uploadFile = async (file: File | Blob) => {
    try {
      ...
      // Send the FormData object to your Flask API endpoint
      const response = await fetch(`${FLASK_API_URL}/upload-file`, {
        method: 'POST',
        body: formData,
      });
  ...
  const stopRealTimeDetection = () => {
    if (intervalId) {
      ...
      setTimeout(() => {
        setShowModal(true);
        console.log('Showing action over time modal');
      }, 2000);
    }
  };
      ...
      datasets: [
        {
          ...
          data: dataValues,
          backgroundColor: labels.map(label => chartDataStyle(label as keyof ActionData)),
          borderColor: labels.map(label => chartDataStyle(label as keyof ActionData)),
        },
      ],
    };
    return chartData;
  ...
  const chartData2 = actionHistory.length > 0 ? {
    ...
    <>
      ...
      <main className="min-h-screen bg-black text-white">
        <div className="container mx-auto p-4">
          <h1 className="text-5xl font-bold text-center mb-6" style={{ marginTop: '40px' }}>Action Recognition</h1>
          ...
          <div className="flex flex-col lg:flex-row justify-center items-start gap-8 mt-6 lg:mt-24">
            <div className="webcam-container lg:w-1/2 space-y-4">
              ...
              <div className="webcam-style w-full rounded overflow-hidden relative" style={{ height: '420px' }}>
                {imagePreview ? (
                   <img src={imagePreview} alt="Preview" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                ) : isCameraEnabled ? (
                  <Webcam ref={webcamRef} audio={false} screenshotFormat="image/jpeg" width="100%" height="100%" />
                ) : (
            ...
            <div className="prediction-container lg:w-1/2" style={{ marginTop: '40px', display: 'flex', flexDirection: 'column', alignItems: 'stretch' }}>
              <div className="graphical-visualization rounded-lg border border-white shadow-2xl overflow-hidden" style={{ height: '420px', width: '100%', maxWidth: '720px', marginBottom: '1rem' }}>
                <h2 className="text-xl font-semibold text-center mt-2 mb-4">Predicted Action: {highestActionName ? `${highestActionName} (${(highestActionPercentage * 100).toFixed(2)}%)` : 'Calculating...'}</h2>
                {actionData ? (
                  <Bar data={getChartData(actionData)} options={options} />
                ) : (
                  <span className="flex justify-center">No action detected yet.</span>
                )}
              </div>
              <div className="flex space-x-2 mt-auto">
                <input type="file" ref={fileInputRef} onChange={handleFileSelect} className="file-input" style = {{width : '100%'}}/>
        ...
        {showModal && (
          <>
            <div style={{
              ...
              <h2 style={{ marginBottom: '20px', fontSize: '24px', color: '#333' }}>Action Over Time</h2>
              <Line key={actionHistory.length} data={chartData2} options={options2} />
